...
_SUMMARY_TTL = 20
_SACCT_TTL = 300
_JOBSTATS_MAX_FILE_BYTES = 50 * 1024 * 1024
_JOBSTATS_MAX_POINTS = 5000
_JOBSTATS_MAX_SAMPLES = 200000
_JOBSTATS_SECTOR_BYTES = 512
_PAST_JOB_SORTS = {
    "newest",
    "oldest",
    "status_asc",
...
def _open_jobstats_log(path):
    ...
    if not path.is_file() or path.stat().st_size > _JOBSTATS_MAX_FILE_BYTES:
        return None
    if path.suffix == ".gz":
...
def _limit_jobstats_points(points):
    ...
    if len(points) <= _JOBSTATS_MAX_POINTS:
        return points
    last = len(points) - 1
    indices = {
        round(position * last / (_JOBSTATS_MAX_POINTS - 1))
        for position in range(_JOBSTATS_MAX_POINTS)
    }
...
def _parse_jobstats_cpu(stream, elapsed_seconds=0):
    ...
    for raw_line in stream:
        ...
        if len(raw_samples) >= _JOBSTATS_MAX_SAMPLES:
            break

    count = len(raw_samples)
    points = []
    for index, (user_percent, free_kb, cache_kb) in enumerate(raw_samples):
        # Match jobstats' own plot: user CPU multiplied by cores on the node,
        # and used memory excluding filesystem cache. vmstat values are KiB.
        points.append({
...
def _parse_jobstats_gpu(stream, elapsed_seconds=0):
    ...
    for row in reader:
        ...
        if len(samples) >= _JOBSTATS_MAX_SAMPLES:
            break
...
def _parse_jobstats_io(stream, elapsed_seconds=0):
    ...
    for raw_line in stream:
        ...
        # vmstat --partition data rows contain four cumulative counters.
    ...
    points = [{
        "elapsed_seconds": _sample_elapsed(index, count, elapsed_seconds),
        "read_bytes": max(0, reads - initial_reads) * _JOBSTATS_SECTOR_BYTES,
        "write_bytes": max(0, writes - initial_writes) * _JOBSTATS_SECTOR_BYTES,
...
def _get_job_detail(job_id):
    output = run_process_output(["scontrol", "show", "job", str(job_id)])
...
def get_jobstats(job_id):
    ...
    try:
        ...
        context = _get_jobstats_context(job_id, current_user)
        if context is None:
            # Deliberately do not reveal whether a job belonging to another user exists.
